...
- (void)setupToolbarActions
{
    ...
    [self.explorerToolbar.showPropertyItem addTarget:self action:@selector(showButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
}
...
//展示选中的控件的名字
- (void)showButtonTapped:(FLEXToolbarItem *)sender {
    UIView *view = self.selectedView;
    NSString *property = [view.superview getPropertyName:view];
    //找到对应的属性名
    if (property.length > 0) {
        NSString *viewProName = [NSString stringWithFormat:@"选中的控件名字是%@",property];
        NSLog(@"property:%@",property);
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:viewProName delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    }
}
